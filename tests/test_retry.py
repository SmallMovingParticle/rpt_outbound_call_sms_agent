...
def test_preproduction_rejects_accelerated_test_mode():
    settings = Settings(app_env="preproduction", test_mode=True)
    assert "TEST_MODE cannot be enabled in pre-production or production" in settings.runtime_errors(
        "api"
    )
