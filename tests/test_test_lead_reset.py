...
def test_test_lead_reset_is_blocked_in_production(monkeypatch):
    monkeypatch.setattr(
        "rpt_agent.cli.get_settings",
        lambda: Settings(app_env="production", test_mode=True),
    )
    with pytest.raises(RuntimeError, match="disabled outside development/test"):
